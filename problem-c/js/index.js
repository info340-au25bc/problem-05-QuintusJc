...
const lostToOpponents = lostGames.map(extractOpponent);
...
const fumbleGames = huskyGames2016.filter(function(game) {
...
const avgScoreDifference = huskyGames2016.reduce(function(total, game) {
  const uwIsHome = game.home === "UW";
  const uwScore = uwIsHome ? game.home_score : game.opponent_score;
  const opponentScore = uwIsHome ? game.opponent_score : game.home_score;
  return total + (uwScore - opponentScore);
